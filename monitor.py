...
class Monitor:
    ...
    def get_crawler_status(self, redis_client):
        """获取所有爬虫状态"""
        try:
            status = redis_client.hgetall(REDIS_KEYS['crawler_status'])
            result = {}
            for crawler_id, status_json in status.items():
                status_data = json.loads(status_json)
                last_update = datetime.fromtimestamp(status_data['last_update'])
                idle_time = (datetime.now() - last_update).seconds
